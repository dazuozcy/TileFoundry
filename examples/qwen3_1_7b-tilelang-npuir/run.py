...
The model is the shipped `qwen3_1_7b` authored HIR (kept verbatim in
`ref_src/`, which is what everything here is measured against); the
implementation that runs is `fast/`, a tilelang-npuir runtime twin whose whole
decode step is one captured torch_npu graph. See `fast/kernels.py` for the
kernels and `fast/engine.py` for why the step can be captured at all.
...
DEFAULT_CKPT = "/home/tilelang/zuochuanuong/weights/qwen3_1_7b"


def pick_device(want: str | None) -> str:
    """The caller's choice, or the current NPU device."""
    if want:
        return want
    import torch
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--ckpt", type=Path, default=Path(DEFAULT_CKPT),
                    help="published checkpoint directory")
    ap.add_argument("--device", default=None, help="runtime device (default: npu)")
